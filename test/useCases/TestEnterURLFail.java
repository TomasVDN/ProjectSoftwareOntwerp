...
class TestEnterURLFail {
	...
	void TestURLMalformed() {
		...
		assertEquals("Error occured. Make sure you entered a valid URL.", ((Text) htmlDocument.getElements().get(0)).getText());
	}
	...
	//check if the error screen is opened
	HTMLDocument htmlDocument = mainWindow.getWindowManager().getMainDialog().getActiveHTMLDocument();
	assertEquals("Error occured. Reason: not a valid Browsr document.", ((Text) htmlDocument.getElements().get(0)).getText());
	}
}
